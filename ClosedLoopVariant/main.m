...
if direction == 1     % forward motion 
    ...
    final.x1 = input_start(1);   % X-Coordinate of Trailer's rear axle
    final.y1 = input_start(2);   % Y-Coordinate of Trailer's rear axle
    theta0_e_d = input_start(3);  % Orientation angle of Truck in degree
    final.theta0 = theta0_e_d*2*pi/360;
    theta1_e_d = input_start(4);  % Orientation angle of Trailer in degree
    final.theta1 = theta1_e_d*2*pi/360;
    phi_e_d = input_start(5);     % Steering angle of Truck in degree
...
% Input is not a number (NaN, isnan()) indicates choosing a random uncertainty
...
if direction == 1   % forward motion
    ...
    start.x1 = input_final(1);   % X-Coordinate of Trailer's rear axle
    start.y1 = input_final(2);   % Y-Coordinate of Trailer's rear axle
    theta0_e_d = input_final(3);  % Orientation angle of Truck in degree
    start.theta0 = theta0_e_d*2*pi/360;
    theta1_e_d = input_final(4);  % Orientation angle of Trailer in degree
    start.theta1 = theta1_e_d*2*pi/360;
    phi_e_d = input_final(5);     % Steering angle of Truck in degree
    start.phi = phi_e_d*2*pi/360; 
    start.x0 = start.x1 + d1*cos(start.theta1); % X-Coordinate of Truck's rear axle
    start.y0 = start.y1 + d1*sin(start.theta1); % Y-Coordinate of Truck's rear axle
end
...
% Assumption: All zeros of the characteristical polynomial = -0.5 (strictly positive)
k0 = 0.0625; 
k1 = 0.5; 
k2 = 1.5;  
k3 = 2; 
...
% With regard to the Trailer's centre of gravity
i=0;
for t=0:0.01:T
    i=i+1;
...
% Plot 1: Forward/Backward Motion - Moving Truck/Trailer (Simulation) 
figure(1)
